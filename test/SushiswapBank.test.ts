...
const SushiswapGoblin = artifacts.require('SushiswapGoblin');
...
contract('SushiswapBank', ([deployer, alice, bob, eve]) => {
  ...
  let poolId: number;
  let goblin: SushiswapGoblinInstance;
  ...
  beforeEach(async () => {
    ...
    await token.mint(deployer, ether('100'));
    await token.mint(alice, ether('100'));
    await token.mint(bob, ether('100'));
    await sushi.mint(deployer, ether('100'));
    await factory.createPair(weth.address, token.address);
    lp = await UniswapV2Pair.at(await factory.getPair(token.address, weth.address));
    ...
    poolId = 0;
    goblin = await SushiswapGoblin.new(
      bank.address,
      masterChef.address,
      router.address,
      poolId,
    ...
    await token.approve(router.address, ether('0.1'));
    await router.addLiquidityETH(token.address, ether('0.1'), '0', '0', deployer, FOREVER, {
      value: ether('1'),
    });
    // Deployer adds 1e17 SUSHI + 1e18 WEI
    await sushi.approve(router.address, ether('1'));
  ...
  it('should has MOCK as fToken in SushiswapGoblin', async () => {
    ...
    expect(await goblin.fToken()).to.be.equal(token.address);
  });
  ...
  it('should allow positions without debt', async () => {
    ...
    await bank.work(
      ...
      web3.eth.abi.encodeParameters(
        ['address', 'bytes'],
        [addStrat.address, web3.eth.abi.encodeParameters(['address', 'uint256'], [token.address, '0'])]
      ),
  ...
  it('should be able to liquidate bad position', async () => {
    ...
    await bank.work(
      ...
      web3.eth.abi.encodeParameters(
        ['address', 'bytes'],
        [liqStrat.address, web3.eth.abi.encodeParameters(['address', 'uint256'], [token.address, '0'])]
      ),
      { from: alice }
    );
  });
  ...
  it('Should deposit and withdraw eth from Bank (bad debt case)', async () => {
    ...
    await router.swapExactTokensForTokens(ether('100'), '0', [token.address, weth.address], deployer, FOREVER);
